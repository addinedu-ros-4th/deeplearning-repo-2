...
class find_object():
    ...
    def calculate_depth(self, image):
        ...
        mtx = np.array([[693.79303454, 0. ,290.04788772],
                  [0., 695.31755488, 248.75886213], 
                  [0.,0.,1.]])
        dist = np.array([[-0.41231278, 0.15145024, 0.00230435, 0.00253762, 0.13721517]])
        ...
        results = self.model.predict(self.calibrated_image, conf=0.6, vid_stride=30, max_det = 5, verbose=False)
        ...
        if results[0].boxes is not None:
            ...
            for object_box, cls_index in zip(object_boxes, cls_indices):
                ...
                if class_name in self.known_widths:
                    ...
                    cv2.putText(self.calibrated_image, f"{(distance*2.278):.2f}cm", (object_box[0], object_box[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
                    # 클래스명 출력
