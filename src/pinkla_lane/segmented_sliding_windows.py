...
from segmented_BEV import birdeye
...
def get_fits_by_sliding_windows(birdeye_binary, line_lt, line_rt, n_windows=9, verbose=False):
    ...
    line_lt.update_line(left_fit_pixel, detected=detected)
    line_rt.update_line(right_fit_pixel, detected=detected)
...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
        ...
        segmented_birdeye, M, Minv = birdeye(segmented_binarized)

        line_lt, line_rt, img_out = get_fits_by_sliding_windows(segmented_birdeye, line_lt, line_rt, n_windows=7)
