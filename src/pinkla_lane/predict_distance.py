...
if __name__ == '__main__':
    ...
    KNOWN_DISTANCE = 11.811  # 객체와의 거리(초기에 검출되는 객체에 한해 입력 요구 / 단위 : inch)
    ...
    while cap.isOpened():
        ...
        results = model.predict(image, conf=0.4, vid_stride=30, max_det = 3, verbose=False)
        ...
        if results[0].boxes is not None:
            ...
            for object_box, cls_index in zip(object_boxes, cls_indices):
                ...
                if class_name in known_widths:
                    ...
                    cv2.rectangle(image, (object_box[0], object_box[1]), (object_box[2], object_box[3]), (255,0,255), 2)
                    cv2.putText(image, f"{(distance*24/9):.2f}cm", (object_box[0], object_box[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
                    
                    # 클래스명 출력
                    cv2.putText(image, class_name, (object_box[0], object_box[1] - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,255), 2)
